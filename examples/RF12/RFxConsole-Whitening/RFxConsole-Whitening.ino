...
word crc = 0x1D0F;
crc = _crc_xmodem_update(crc, 6);
crc = _crc_xmodem_update(crc, 1);
crc = _crc_xmodem_update(crc, 2);
crc = _crc_xmodem_update(crc, 0);
crc = _crc_xmodem_update(crc, 65);
crc = _crc_xmodem_update(crc, 66);
crc = _crc_xmodem_update(crc, 67);
Serial.println(~crc, HEX);

1) the start value is special
2) the calculated CRC is complemented before sending it out
3) the CRC bytes are sent out high-byte first
4) on reception, after all bytes (incl CRC hi+lo), the CRC will be 0x1D0F again
...
void SX1232RadioComputeWhitening( volatile uint8_t *buffer, uint16_t bufferSize ) { 
// Sourced from http://www.semtech.com/images/datasheet/AN1200.18_STD.pdf
  uint8_t i = 0; 
  uint16_t j = 0; 
  uint8_t WhiteningKeyMSBPrevious = 0; 
  uint8_t revertedWhiteningKeyLSB = 0; 
  uint8_t WhiteningKeyLSB = 0xFF; // Added JOH
  uint8_t WhiteningKeyMSB = 1;    // Added JOH, 9 bits required

  revertedWhiteningKeyLSB = WhiteningKeyLSB;  // WhiteningKeyLSB is 0xFF at init 
//  for( j = 0; j < bufferSize - 1; j++ ) { 
  for( j = 0; j < bufferSize - 0; j++ ) {     // Adjusted JOH
